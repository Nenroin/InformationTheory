...
experimental_values = [
    83.8, 36.7, 81.2, 65.3, 42.0, 50.1, 92.4, 85.6, 83.5, 72.1,
    76.3, 69.4, 56.5, 47.3, 23.1, 65.1, 56.4, 25.1, 83.4, 59.5,
    41.6, 24.4, 11.8, 70.1, 57.1, 87.4, 69.1, 30.1, 65.4, 49.9,
    55.9, 74.2, 32.3, 92.1, 20.7, 35.3, 60.2, 32.1, 74.5, 31.4,
    38.6, 92.5, 45.8, 58.4, 53.4, 35.9, 48.3, 60.1, 46.2, 37.4,
    50.9, 39.9, 45.3, 74.4, 21.2, 29.7, 45.8, 41.4, 56.9, 65.8,
    54.7, 70.9, 15.1, 47.7, 12.7, 80.9, 74.9, 65.7, 47.5, 39.1,
    48.6, 87.1, 36.1, 47.2, 26.1, 27.3, 47.4, 28.1, 83.9, 76.2,
    66.3, 51.4, 11.6, 30.9, 25.7, 76.7, 22.6, 19.3, 64.1, 54.2,
    56.7, 20.3, 92.6, 29.5, 52.0, 29.9, 75.1, 20.5, 76.5, 18.4
]
...
n = 100
Y = [7.5, 8.0, 8.5, 9.0, 9.5, 10.0, 10.5, 11]
X = [115, 120, 125, 130, 135, 140]
m_x = [9, 15, 19, 25, 19, 13]
m_y = [1, 6, 24, 31, 18, 15, 3, 2]
...
print("Находим x, σᵪ, y, σᵧ. Так как СВ Х и Y имеют интервальные",
      "\nраспределения, то находим середины соответствующих интервалов xᵢ",
      "\nи yⱼ. Для облегчения расчетов введем так называемые условные",
      "\nварианты uᵢ u vᵢ, i = 1.7, j = 1.6.",
      f"\nuᵢ = (xᵢ - C₁)/h₁ = (xᵢ - {round(С_1, 4)})/{h_1}   i = 1.7",
      f"\nvⱼ = (yⱼ - C₂)/h₂ = (yⱼ - {round(С_2, 4)})/{h_2}   j = 1.6")
...
while i < len(X) - 1:
    ...
    print(f"{X[i]} — {X[i + 1]}   x{i + 1} = {x[i]}    u{i + 1} = {u[i]}   m{i + 1} = {round(m_x[i], 4)}",
...
while i < len(Y) - 1:
    ...
    print(f"{Y[i]} — {Y[i + 1]}   y{i + 1} = {y[i]}    v{i + 1} = {v[i]}   m{i + 1} = {round(m_y[i], 4)}",
